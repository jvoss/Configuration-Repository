...
backing up device configurations. A driver is used to customize what
is being archived off the device. However the sysDescr 
string can be queried via SNMP to determine which type of host the device
is. This is the default method for determining a driver when none is specified.
